...
(function() {
  ...
  Record = (function(_super) {
    ...
    Record.configure = function(options) {
      ...
      return options.redis.on('connect', (function(_this) {
        return function() {
          ...
          _this.setClient(options.redis);
          _this.setPrefix(options.prefix);
          return (_ref = options.connect) != null ? _ref.call(_this) : void 0;
        };
      })(this));
    };

    Record.model = function(name, options) {
      var collection, model, orig;
      ...
      if (model.collectionDefinition) {
        ...
        collection.forEach(function(col) {
          ...
          return Record.collections[name] = collection;
        });
      }
      orig = model.find;
    ...
    Record._extends = {
      collection: function(id) {
        ...
        if (this.collectionDefinition == null) {
          return this.logError("No collection definition found");
        }
        name = "" + this.modelName + ":collection:" + id;
        collection = this.collections[name];
        if (!collection) {
          collection = extend({}, this);
          collection.prototype.modelName = name;
          this.collections[name] = view;
        }
        return collection;
      },
      getClient: function() {
      ...
      getHashKey: function(id) {
        return "" + Record.prefix.hash + this.modelName + ":" + id;
      },
      ...
      count: function(criteria, callback) {
        ...
        if (typeof criteria === 'function') {
          ...
          criteria = null;
          m = new this;
          return Record.client.scard(Record.prefix.idsets + m.modelName, function(err, result) {
            if (err) {
              return callback(err);
            }
                ...
                set_update = (function(_this) {
                  return function(prop) {
                    ...
                    if (_this.properties[prop].unique) {
                      ...
                      return multi.setnx("" + Record.prefix.unique + _this.modelName + ":" + prop + ":" + _this.properties[prop].value, id);
                    } else {
      ...
      deindex: function(properties, callback) {
        ...
        return properties.forEach((function(_this) {
          return function(p, idx) {
            return Record.client.keys("" + Record.prefix.unique + _this.modelName + ":" + p + ":*", function(err, unique_keys) {
              deletes = unique_keys;
              return Record.client.keys("" + Record.prefix.index + _this.modelName + ":" + p + ":*", function(err, index_keys) {
                deletes = deletes.concat(index_keys);
                return Record.client.keys("" + Record.prefix.scoredindex + _this.modelName + ":" + p + ":*", function(err, scoredindex_keys) {
    ...
    Record.backend = {
      connect: function(req, res, next) {
        ...
        StatsHandler = function(req, res, next) {
          return Record.client.info(function(err, result) {
            if (!err) {
